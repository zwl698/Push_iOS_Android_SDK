...
func (c *Client) PushWithContext(ctx Context, n *Notification) (*Response, error) {
	...
	if err != nil {
		tklog.WriteErrorlog("ios httpres,err=%s.", err.Error())
		return nil, err
	}
	...
	tklog.WriteInfolog("templog.status:%d.apnsid=%s.url=%s", response.StatusCode, response.ApnsID,url)
	bt, err := ioutil.ReadAll(httpRes.Body)
//	tklog.WriteInfolog("templog,read all finish.")
	if err != nil {
		return response, err
	}
	str := string(bt)
	str, err = strconv.Unquote(str)
//	tklog.WriteInfolog("templog,Unquote all finish.")
	if err != nil {
		str = string(bt)
	}
	if str != "" {
//		tklog.WriteInfolog("templog,Unmarshal begin.")
		err = json.Unmarshal([]byte(str), response)
//		tklog.WriteInfolog("templog,Unmarshal finish.")
		if err != nil {
			return response, err
		}
	}
